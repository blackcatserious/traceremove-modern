...
function extractTextFromBlock(block: { type: string; [key: string]: unknown }): string {
  switch (block.type) {
    case 'paragraph':
      return (block.paragraph as { rich_text: Array<{ plain_text: string }> }).rich_text.map(text => text.plain_text).join('');
    case 'heading_1':
      return (block.heading_1 as { rich_text: Array<{ plain_text: string }> }).rich_text.map(text => text.plain_text).join('');
    case 'heading_2':
      return (block.heading_2 as { rich_text: Array<{ plain_text: string }> }).rich_text.map(text => text.plain_text).join('');
    case 'heading_3':
      return (block.heading_3 as { rich_text: Array<{ plain_text: string }> }).rich_text.map(text => text.plain_text).join('');
    case 'bulleted_list_item':
      return '• ' + (block.bulleted_list_item as { rich_text: Array<{ plain_text: string }> }).rich_text.map(text => text.plain_text).join('');
    case 'numbered_list_item':
      return '1. ' + (block.numbered_list_item as { rich_text: Array<{ plain_text: string }> }).rich_text.map(text => text.plain_text).join('');
    case 'quote':
      return '> ' + (block.quote as { rich_text: Array<{ plain_text: string }> }).rich_text.map(text => text.plain_text).join('');
    case 'code':
      return '```\n' + (block.code as { rich_text: Array<{ plain_text: string }> }).rich_text.map(text => text.plain_text).join('') + '\n```';
    default:
      return '';
  }
}
...
export async function reindexPersona(personaId: string, notionDbId: string, sitemapUrl: string): Promise<void> {
  try {
    ...
    try {
      console.log(`Reindexing persona ${personaId} - existing vectors will be overwritten`);
    } catch (error) {
      console.log('No existing vectors to delete or error deleting:', error);
    }
